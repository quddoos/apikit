...
public class JsonSchemaManager {
	...
	public EntitySet getEntitiesFromSchema() throws JsonSyntaxException, FileNotFoundException, IOException, GatewayMetadataMissingFieldsException,
		...
		for (int i = 0; i < schemas.length(); i++) {
			if (schemas.getJSONObject(i).keySet().size() != 1) {
				// wrong format?
			}

			String entityName = (String) schemas.getJSONObject(i).keySet().toArray()[0];
			JSONObject schema = schemas.getJSONObject(i).getJSONObject(entityName);
	...
	private List<EntityProperty> parseEntityProperties(JSONObject properties) throws GatewayMetadataMissingFieldsException {
		List<EntityProperty> entityProperties = new ArrayList<EntityProperty>();
		if (properties != null) {
			for (int j = 0; j < properties.keySet().size(); j++) {
				String propertyName = (String) properties.keySet().toArray()[j];
				...
				EntityProperty newEntityProperty = new EntityProperty(propertyName, sample, type, nullable, length, description, key);
